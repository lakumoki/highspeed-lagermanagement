...
// Hilfsfunktion: Nächste unverarbeitete Palette mit gegebener Nr. finden
function findUnprocessedPalette(nr, usedIds) {
  ...
  `).all(nr);
  return all.find(p => !usedIds.has(p.id)) || null;
}
...
router.post('/', (req, res) => {
  ...
  const palette = findUnprocessedPalette(paletten_nr, new Set());
  ...
  if (neuerPlatz.belegt && neuerPlatz.typ !== 'Gang' && neuerPlatz.typ !== 'Block' && neuerPlatz.id !== palette.alter_platz_id) {
  ...
  db.prepare('INSERT INTO protokoll (aktion, details, benutzer, zeitstempel) VALUES (?,?,?,?)').run('Umlagerung', `Palette ${paletten_nr} (ID ${palette.id}) | Von: ${palette.von_platz || '?'} → Nach: ${neuerPlatz.bezeichnung} | Kunde: ${palette.kunde_id ? (db.prepare('SELECT name FROM kunden WHERE id=?').get(palette.kunde_id)?.name || '?') : '—'} | Keine Berechnung`, benutzer, jetzt);
...
router.post('/bulk', (req, res) => {
  ...
  const transaction = db.transaction(() => {
    for (const nr of paletten_nummern) {
      const palette = findUnprocessedPalette(nr, usedIds);
      if (!palette) { errors.push(`${nr}: nicht gefunden (oder alle mit dieser Nr. bereits verarbeitet)`); continue; }
      usedIds.add(palette.id);
